...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
  ...
  vec3 scanAura = uScanColor * combinedAura;
    ...
    color = gridCol + (1.0 - lineVis) * (scanCol + scanAura);
...
export const GridScan = ({
  ...
  useEffect(() => {
    ...
    if (scanOnClick && el) el.addEventListener('click', onClick);
    return () => {
      window.removeEventListener('mousemove', onMove);
      document.documentElement.removeEventListener('mouseleave', onLeave);
      if (scanOnClick && el) el.removeEventListener('click', onClick);
    };
  ...
  useEffect(() => {
    ...
    if (m) {
      ...
      u.uBloomOpacity.value = active ? Math.max(0, bloomIntensity) : 0;
      u.uNoise.value = Math.max(0, noiseIntensity);
      u.uScanGlow.value = scanGlow;
      u.uScanOpacity.value = active ? Math.max(0, Math.min(1, scanOpacity)) : 0;
    ...
    if (bloomRef.current) {
      bloomRef.current.blendMode.opacity.value = active ? Math.max(0, bloomIntensity) : 0;
